document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (!token) {
            messageContainer.textContent = 'ERROR: You are not logged in. Please log in to create a post.';
        ...
        if (!postType || !title) {
            messageContainer.textContent = 'Please select a post type and enter a title.';
            messageContainer.style.color = 'red';
            return;
        }
        ...
        if (postType === 'Poll') {
            const options = Array.from(document.getElementsByClassName('poll-option-input'))
                                 .map(input => input.value)
                                 .filter(value => value.trim() !== '');
            if (options.length < 2) {
                messageContainer.textContent = 'Polls must have at least two non-empty options.';
                messageContainer.style.color = 'red';
                return;
            }
        ...
        try {
            ...
            if (response.ok) {
                messageContainer.textContent = 'Post created successfully! Redirecting to feed...';
            ...
            messageContainer.textContent = 'A network error occurred. Could not connect to the server.';
            messageContainer.style.color = 'red';
        }
    });
});